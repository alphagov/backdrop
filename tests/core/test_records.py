...
class TestRecord(unittest.TestCase):
    ...
    def test__timestamp_is_returned_as_datetime(self):
        incoming_data = {
            'foo': 'bar',
            '_timestamp': datetime.datetime(2013,2,2,0,2,0, tzinfo=pytz.UTC)
        }
        some_record = Record(incoming_data)

        assert_that(some_record.data['_timestamp'], is_(instance_of(datetime.datetime)))
        assert_that(
            some_record.data['_timestamp'],
            is_(datetime.datetime(2013,2,2,0,2,0,tzinfo=pytz.UTC))
        )
    ...
    def test_data_with__timestamp_gets_a__week_start_at(self):
        incoming_data = {
            'foo': 'bar',
            '_timestamp': datetime.datetime(2013,2,2,0,0,0, tzinfo=pytz.UTC)
        }
        some_record = Record(incoming_data)
    ...
    def test__week_start_at_is_always_a_week_before__timestamp(self):
        incoming_data_1 = {
            'foo': 'bar',
            '_timestamp': datetime.datetime(2013,2,7,0,0,0, tzinfo=pytz.UTC)
            }
        incoming_data_2 = {
            'foo': 'bar',
            '_timestamp': datetime.datetime(2013,3,14,0,0,0, tzinfo=pytz.UTC)
        }
    ...
    def test__week_start_at_gets_time_zeroed(self):
        incoming_data = {
            'foo': 'bar',
            '_timestamp': datetime.datetime(2013,2,7,7,7,7, tzinfo=pytz.UTC)
        }
    ...
    def test_to_mongo(self):
        record = Record({
            'name': 'bob',
            '_timestamp': datetime.datetime(2013,4,4,4,4,4,tzinfo=pytz.UTC)
        })
