...
class PeriodData(object):
    def add(self, document):
        self.datum = [self.__create_datum(document)]
...
class TestPeriodData(unittest.TestCase):
    def test_adding_mongo_document_to_collection_expands_week_start_at(self):
        ...
        assert_that(len(period_data.datum), is_(1))
        assert_that(period_data.datum[0], has_entry("_count", 42))
        assert_that(period_data.datum[0],
